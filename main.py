...
def get_data(filename):
    taxi_data = pd.read_csv(filename).head(1000)

    return taxi_data


with header:
    st.title('Welcome to my awesome DS project')
    st.text('In this project I look into the trainsactions of taxis in NYC ...')


with dataset:
    st.header('NYC taxi dataset')
    st.text('I found this dataset in blahblah.com ...')

    taxi_data = get_data('data/taxi_data.csv')
    st.write(taxi_data.head())

    st.subheader('A title for the graph')
...
with features:
    ...
    st.markdown('* **First Feature** I created this feature because of this .. I calculated because of this ..')
    st.markdown('* **Second Feature** I created this feature because of this .. I calculated because of this ..')


with model_training:
    st.header('time to train the model')
    st.text('Have some model info')
